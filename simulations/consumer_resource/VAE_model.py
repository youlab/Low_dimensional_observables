...
def run(var_type,n_target,n_embedding,X_train,X_test,model_index,trial=1):
    ...
    for ix_epoch in range(EPOCHS):
        ...
        save_path = save_dir / f"mcrm_{var_type}_T{n_target}_E{n_embedding}_trial{trial}.pth"

        torch.save(model.state_dict(), save_path)
    print(f"{n_target}-target {var_type} training instance {trial} finished, with starting MSE {test_losses[0]:1.1e}, and ending error {test_losses[-1]:1.1e}")
