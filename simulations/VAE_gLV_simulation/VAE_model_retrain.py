...
def retrain(n_background,n_target,n_embedding,X_train,X_test,glv_type,sim_type,model_index):
    # set the random seed so the three trials have different initial model weights
    seed = 1000 * n_target + 10*n_embedding
    ...
    torch.save(model.state_dict(),
               "./vae_models/%sgLV/I%i/%s_B%i_T%i_E%i_retrain.pth"
               % (glv_type, model_index, sim_type, n_background, n_target, n_embedding)
               )
