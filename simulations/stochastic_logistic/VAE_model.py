...
def get_data(n_target, model_index):
    X_train = np.load(f"./saved_sims/I{model_index}/T{n_target}_train.npy")
...
def run(n_target,n_embedding,X_train,X_test,model_index,trial=1,save=True):
    ...
    if save:
        for ix_epoch in range(EPOCHS):
            ...
            save_path = save_dir / f"slogistic_T{n_target}_E{n_embedding}_trial{trial}.pth"

            torch.save(model.state_dict(), save_path)
            print(f"{n_target}-target stochastic logistic model training instance {trial} finished, with starting MSE {test_losses[0]:1.1e}, and ending error {test_losses[-1]:1.1e}")
