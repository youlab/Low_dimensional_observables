...
def get_data(n_target, model_index):
    X_train = np.load(f"./saved_sims/I{model_index}/bgLV_T{n_target}_random_train.npy")
    X_test = np.load(f"./saved_sims/I{model_index}/bgLV_T{n_target}_random_test.npy")

    return X_train, X_test

def get_indiv_target(target_id, model_index):
    X_train = np.load(f"./saved_sims/I{model_index}/bgLV_all_train.npy")[:,target_id:target_id+1,:]
    X_test = np.load(f"./saved_sims/I{model_index}/bgLV_all_test.npy")[:,target_id:target_id+1,:]
...
def run(n_target,n_embedding,X_train,X_test,model_index,trial=1,save=True):
    ...
    if save:
        for ix_epoch in range(EPOCHS):
            ...
            save_dir = Path(f"./vae_models/I{model_index}")
            save_dir.mkdir(parents=True, exist_ok=True)

            save_path = save_dir / f"network_T{n_target}_E{n_embedding}_trial{trial}.pth"

            torch.save(model.state_dict(), save_path)
            print(f"{n_target}-target gLV model with sparse network training instance {trial} finished, with starting MSE {test_losses[0]:1.1e}, and ending error {test_losses[-1]:1.1e}")
...
def run_indiv(target_id,n_embedding,X_train,X_test,model_index,trial=1,save=True):
    # set the random seed so the three trials have different initial model weights
    seed = 1000 * target_id + 10*n_embedding + trial
    torch.manual_seed(seed)
    ...
    if save:
        for ix_epoch in range(EPOCHS):
            ...
            save_path = save_dir / f"indiv_s{target_id}_E{n_embedding}_trial{trial}.pth"

            torch.save(model.state_dict(), save_path)
            print(f"target #{target_id} training instance {trial} finished, with starting MSE {test_losses[0]:1.1e}, and ending error {test_losses[-1]:1.1e}")
        return train_losses, test_losses
    else:
        for ix_epoch in trange(EPOCHS):
            train_err = train_model(model, train_loader, optimizer)
