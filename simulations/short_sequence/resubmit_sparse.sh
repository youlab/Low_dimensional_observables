...
#SBATCH --job-name=sparse_retrain
#SBATCH --mail-type=END,FAIL
#SBATCH --mail-user=kw367@duke.edu
#SBATCH --output=/hpc/group/youlab/wendyw/logs/slurm/sparse_retrain_%j.out
...
mkdir -p ./slurm_outputs/sparse_v2
  ...
  for NT in 2 3 5 8 10; do
    for N_OBS in 5 10 20 50; do
      for TRIAL in 1 2 3 4 5; do
        OUT="./slurm_outputs/sparse_v2/sparse_M${MODEL}_NT${NT}_NOBS${N_OBS}_T${TRIAL}.out"
        maybe_run "$OUT" \
          train_sparse.py "${MODEL}" "100" "${NT}" "random" "${TRIAL}" "${N_OBS}"
      done
    done
  done
done
...
echo "=============================="
echo "All done: $(date)"
echo "Sparse .pth files saved:"
ls ./vae_models/bgLV/I*/sparse/sparse_*.pth 2>/dev/null | wc -l
echo "=============================="