...
class VAE_variant2(nn.Module): # double linear layer variational autoencoder
...
def run(n_background,n_target,n_embedding,X_train,X_test,glv_type,sim_type,model_index,trial=1,save=True):
    ...
    model = VAE_variant2(n_target, n_embedding)
    model.to("cuda:0");
    model.train()
    train_losses = []
    test_losses = []
    ...
    if save:
        save_path = Path(
            f"./VAE_v2/{glv_type}gLV/I{model_index}/{sim_type}_B{n_background}_T{n_target}_E{n_embedding}_trial{trial}.pth"
        )
