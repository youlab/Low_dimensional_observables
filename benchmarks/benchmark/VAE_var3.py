...
class VAE_variant3(nn.Module): # VAE with 16 channels in the conv layers
    ...
    def __init__(self, n_target, latent_dim):
        ...
        self.mean_map = nn.Sequential(
            nn.Linear(self.channels * self.T, self.latent_dim),
        )
...
def run(n_background,n_target,n_embedding,X_train,X_test,glv_type,sim_type,model_index,trial=1,save=True):
    ...
    model = VAE_variant3(n_target, n_embedding)
    model.to("cuda:0");
    model.train()
    train_losses = []
    test_losses = []
    ...
    if save:
        save_path = Path(
            f"./VAE_v3/{glv_type}gLV/I{model_index}/{sim_type}_B{n_background}_T{n_target}_E{n_embedding}_trial{trial}.pth"
        )
