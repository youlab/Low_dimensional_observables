# Levina-Bickel intrinsic-dimension estimates for the bounded gLV simulations (fixed background).
#
# Data: all 10,000 simulations of each dataset, rebuilt by concatenating the train (8,000) and
# test (2,000) splits in
#   simulations/VAE_gLV_simulation/saved_sims/bgLV/I*/bgLV_B*_T*_fixed_{train,test}.npy
...
sim_type="fixed"
n_ori=100
NT = np.array([2,3,4,5,6,7,8,9,10])
...
for i,model_index in enumerate(np.arange(1,4,1)):
    ...
    for j,n_target in enumerate(NT):
        ...
        print(f"model {model_index}, {n_target} target finished")
    if not use_cache:
...
fig.subplots_adjust(left=0.1,bottom=0.1,right=0.95,top=0.88,hspace=0.1,wspace=0.1)
