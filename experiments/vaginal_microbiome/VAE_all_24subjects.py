...
def run(n_target,n_embedding,X_train):
    lr = 5e-4
    lr_decay = 0.95
    batch_size = 64
    EPOCHS = 30
    ...
    for ix_epoch in range(EPOCHS):
        ...
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr
    print("%i-dim, %i-target model training finished, with starting MSE %1.1e, and ending error %1.1e"%
          (n_embedding,n_target,train_losses[0],train_losses[-1]))
    torch.save(model.state_dict(),
           "./vae_models_CV/T%i_E%i_full.pth"%(n_target,n_embedding))
