...
trial=int(sys.argv[4]) # run three trials
...
for n_embedding in N_EBD:
    model, train_loss, test_loss = run(sample,n_target,n_embedding,X_train,X_test,test_rep,trial)
    TRAIN_LOSS[n_embedding]=train_loss
    TEST_LOSS[n_embedding]=test_loss

loss_data = [TRAIN_LOSS,TEST_LOSS]
filename = ("./vae_models_CV/%s_T%i_rep%i_trial%i_traindoc.pkl"
            % (sample, n_target, test_rep, trial))
