...
def get_data(sample,n_target,test_rep):
    ...
    X_train = []
    for i in train_arr:
        rep = np.load("./lab_microbiome_datasets/%s/%s_T%i_rep%i.npy" %
                (sample, sample, n_target, i))
        X_train.append(rep)
    X_train = np.vstack(X_train)
    X_test = np.load("./lab_microbiome_datasets/%s/%s_T%i_rep%i.npy" %
            (sample, sample, n_target, test_rep))
    return X_train, X_test

def run(sample,n_target,n_embedding,X_train,X_test,test_rep):
    lr = 5e-4
    lr_decay = 0.95
    batch_size = 64
    EPOCHS = 30
    ...
    torch.save(model.state_dict(),
           "./vae_models/%s_T%i_E%i_rep%i.pth"%
           (sample,n_target,n_embedding,test_rep))
