...
def run(sample,n_target,n_embedding,X_train,X_test,test_rep,trial):
    # set the random seed so the three trials have different initial model weights
    seed = 1000 * n_target + 100*n_embedding + 10*test_rep + trial
    ...
    torch.save(model.state_dict(),
           "./vae_models_CV/%s_T%i_E%i_rep%i_trial%i.pth"%
           (sample,n_target,n_embedding,test_rep,trial))
